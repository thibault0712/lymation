<?php
   ...
   // si la session existe pas soit si l'on est pas connecté on redirige
    ...
    if(isset($_GET['id']) && isset($_GET['page'])){
        ...
        $req->execute(array(htmlspecialchars($_GET['id']), $_SESSION['user'], 'chaud'));
        $data = $req->fetch();
        $row = $req->rowCount();
        if($row == 1){
            ...
            $number->execute(array(htmlspecialchars($_GET['id']), 'chaud'));
            $number = $number->fetch();
            $req = $db->prepare('UPDATE like_number SET number = ? WHERE id = ? AND name = ?');
            $req->execute(array($number['number']-1, htmlspecialchars($_GET['id']), 'chaud'));
